 ...
 */
public class CodeRunForCpp implements CodeRun {
    ...
    public ExecuteCodeResponse run(ExecuteCodeRequest request) {
        ...
        String fileName = TranCodeToFile.tranCode(code, location + UUID.randomUUID().toString() + ".cpp");
        String prefixName = fileName.substring(0, fileName.lastIndexOf("."));
        try {
            Process process = new ProcessBuilder("g++", fileName, "-o", prefixName + ".exe").start();
